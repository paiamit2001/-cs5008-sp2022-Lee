...
// void selection_sort_2arr(int* source, int* dest, boool* valid) {
//	int smallest = 0;
//	for(int i = 0; i < Limit; i++) {
//		for(j = 0; j < Limit; j++) {
//			if(valid[j]) {
//				if(sourc[j] < source[smallest]) {
//					smallest = j;
//				}
//				dest[i] =source[smallest];
//				valid[smallest] = false;
...
// i: i is the current position you check.
// j: j is the possible smallest value.
// // valid array is used to log whether a value can be the smallest value.

void selection_sort_1arr(int *source)
{
  int smallest; // current smallest element index
